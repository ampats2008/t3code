...
describe("classifyToolDisplayMode", () => {
  it("returns rich-agent for collab_agent_tool_call", () => {
    expect(classifyToolDisplayMode(makeWorkEntry({ itemType: "collab_agent_tool_call" }))).toBe(
      "rich-agent",
    );
  });

  it("returns rich-bash for command_execution", () => {
    expect(classifyToolDisplayMode(makeWorkEntry({ itemType: "command_execution" }))).toBe(
      "rich-bash",
    );
  });

  it("returns rich-bash for requestKind command without itemType", () => {
    expect(classifyToolDisplayMode(makeWorkEntry({ requestKind: "command" }))).toBe("rich-bash");
  });
  ...
  it("returns simple for file_change without matching requestKind", () => {
    expect(classifyToolDisplayMode(makeWorkEntry({ itemType: "file_change" }))).toBe("simple");
  });

  it("returns simple for mcp_tool_call", () => {
    expect(classifyToolDisplayMode(makeWorkEntry({ itemType: "mcp_tool_call" }))).toBe("simple");
  });

  it("returns simple for dynamic_tool_call", () => {
    expect(classifyToolDisplayMode(makeWorkEntry({ itemType: "dynamic_tool_call" }))).toBe(
      "simple",
    );
  });

  it("returns simple for web_search", () => {
    expect(classifyToolDisplayMode(makeWorkEntry({ itemType: "web_search" }))).toBe("simple");
  });
