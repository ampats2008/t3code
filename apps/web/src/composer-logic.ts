...
export type ComposerSlashCommand = "model" | "plan" | "default" | "rename";
...
const _SLASH_COMMANDS: readonly ComposerSlashCommand[] = ["model", "plan", "default", "rename"];
...
export function parseStandaloneComposerSlashCommand(
  ...
  const match = /^\/(plan|default|rename)\s*$/i.exec(text.trim());
  if (!match) {
    return null;
  }
  const command = match[1]?.toLowerCase();
  if (command === "plan") return "plan";
  if (command === "rename") return "rename";
  return "default";
}
